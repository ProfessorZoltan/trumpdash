...
// a sign before each, and every hazard carries a death message that says what to do differently.
(function (root) {
  ...
  const def = {
    ...
    deathMsgs: {
      tut_jump: ['Tap when you hear the clap.', 'A touch earlier: the jump peaks over the spike.', 'Space, click or tap the screen. On the clap.'],
      ...
      spike: ['Same rule upside down: tap on the clap.', 'Sad! Try again.'],
    ...
    build(api) {
      const { S, spikeRaw, blockRaw, slabRaw, O, PAD, COIN, FLIP, SIGN, SCENE, GOAL, bx, B, CY } = api;
      // ================= INTRO (bars 0-1): the lawn =================
      SCENE(1.2, 'whitehouse');
      SIGN(3.2, 'HOW TO PLAY', 'a walk through the moves');
      SIGN(6.4, 'LISTEN FOR THE CLAP', 'every jump lands on it');
      // ================= JUMP (bars 2-8): one tap per spike, then hold =================
      SIGN(9.4, 'TAP TO JUMP', 'space, click, or tap the screen');
      ...
      SIGN(68.4, 'FLIP-FLOP PORTAL', 'jump into it and gravity flips');
      FLIP(72, 'FLIP-FLOP');
      ...
      SIGN(92.4, 'PRACTICE MODE', 'press P: checkpoints every few bars');
      ...
      SIGN(114.4, 'TAPS FEEL LATE?', 'use SYNC on the menu, once');
      // ================= FINALE (bars 29-30) =================
      S(116, 1, 'tut_jump'); S(118, 1, 'tut_jump');
      SIGN(120.4, "THAT'S ALL", 'now go pick a level');
      GOAL(124);
    },
    ...
    music: {
      step(A, i) {
        ...
        if (sub === 0 && drums) {
          ...
          if ((bib === 1 || bib === 3) && busy) A.snare(t, 0.28); // a snare, not a clap: the clap is the jump cue
        }
